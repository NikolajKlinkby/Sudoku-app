...
int main() {
    ...
    std::string text_depos = ".text_depos";

    text_depos_write.open (text_depos,std::ios::app);
    text_depos_write.close();
    std::filesystem::remove(text_depos);
    text_depos_write.open (text_depos,std::ios::app);
    text_depos_write.close();
    ...
    while (!done){
        ...
        ImGui::Begin(win_name,&tool_active,ImGuiWindowFlags_NoCollapse|ImGuiWindowFlags_MenuBar);
        ...
        char buf[10];
        ImGui::InputTextMultiline("Line",buf, IM_ARRAYSIZE(buf));
        ...
        text_depos_read.open (text_depos,std::ios::in);
